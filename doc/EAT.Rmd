...
```{r plotEAT, fig.width = 10.5, fig.height = 9, fig.align = 'center', eval = FALSE}
plotEAT(object)
```
...
In this example, we have used 13 inputs for modeling. `EAT` allows a feature selection by `rankingEAT` or `rankingRFEAT` (more details later). `rankingEAT` calculates variable importance for an `EAT` object. The user can specify the number of decimal units (`r`), include a barplot with the scores of importance (`barplot`) and a broken line to display in the graph (`threshold`).
...
Now, we carry out a feature selection by the score obtained in `rankingEAT`. We set `threshold` in 70 and we select those features whose importance is greater than this value, that is, **S**, **GDP_PPP**, **NBMC**, **HW** and **I**. We repeat the analysis:
...
Efficiency scores are numerical values that indicate the degree of efficiency of a set of DMUs displayed in `data` with `x` and `y` the indexes for inputs and outputs respectively. Both `efficiencyEAT` and `efficiencyFDH` require entering the used distance by the `score_model` argument. In case of `efficiencyEAT`, the `EAT` object is necessary. 
...
```{r ranking_RFEAT_ex, fig.width = 10.5, fig.height = 8, fig.align = 'center'}
