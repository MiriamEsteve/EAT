...
date: "`r Sys.Date()`"
author: "Miguel Hernández University"
output: 
...
* A **D**ecision **M**aking **U**nit (**DMU**) is a observation of the dataset whose efficiency is to be assessed. A DMU will be efficient when it is located at the production frontier and will have room for improvement when it is located in the area underlying the border. A non-efficient DMU can become efficient through the following options: 

  * Reducing the amount of input keeping the same level of output.

  * Increasing the amount of output keeping the same level of input. 

  * Reducing the level of input and increased the level of output at the same time.

The Efficiency Analysis Trees modeling is aimed at capturing the maximum trends of a set of outputs. For this purpose, DMUs under the same resource paradigm are grouped into nodes and subsequently the maximum expected output is estimated for each node. The Efficiency Analysis Trees predictor results in a monotonic increasing frontier with a stepped form where each of these steps corresponds to a node of the tree containing homogeneus DMUs with efficient and non-efficient output levels.

In this section, an **EAT** model, an **RFEAT** model and a **FDH** model refer to a modeling carried out using Efficiency Analysis Trees technique, Random Forest + Efficiency Analysis Trees technique and Free Disposal Hull technique respectively. The functions developed in the `EAT` library will always be oriented to one of the 3 previous models (EAT, RFEAT or FDH) and can be divided into 7 categories depending on its purpose:
...
The `print` function on an `EAT` object returns a table with the following elements referring to the leaf nodes of the tree: 

* **id**: leaf node index.
* **n(t)**: number of DMUs in the leaf node.
* **%**: proportion of total DMUs in the leaf node.
* Columns with varibale names (e.g., **S_PISA**): efficient levels of the output in the leaf node.
* **MSE**: mean square error in the leaf node.
...
```{r single.output.frontier, fig.width = 9.7, fig.height = 6, fig.align = 'center'}
...
```{r multioutput.importance, fig.width = 9.7, fig.height = 6, fig.align = 'center'}
...
```{r graph1, fig.width = 9.7, fig.height = 7.5, fig.align = 'center'}
...
```{r graph2, fig.width = 9.7, fig.height = 7.5, fig.align = 'center'}
...
* *numStop*: `numStop` parameter value.
* *fold*: `fold` parameter value.
* *MSE*: mean square error calculated on the test sample with the tree built with the training sample and the numStop and fold values of the corresponding row.
* *length*: number of leaf nodes of the tree.
...
* Tuning for `S_PISA + R_PISA + M_PISA ~ NBMC + WS + S + PS + ABK + AIC + HW + EO + PR + PFC + I + AAE + GDP_PPP` with `numStop = {3, 5, 7, 10}` and `fold = {3, 5, 7}`
...
* Tuning for `S_PISA + R_PISA + M_PISA ~ NBMC + WS + S + HW + AAE` with `numStop = {3, 5, 7, 10}` and `fold = {3, 5, 7}`
...
In both cases, the best tree is given by the hyperparameters `numStop = 5` and `fold = 5`. Since the difference in the MSE error is only 3.84 units, we select the second model as it implies shorter training times. In addition, the selection of variables allows to know which are the features with greater weight in the achievement of high values of the output and therefore those to which we pay more attention.

## Efficiency scores

The efficiency scores are numerical values that indicate the degree of efficiency of a set of DMUs. 

displayed in `data` with `x` and `y` the indexes for inputs and outputs respectively. Both `efficiencyEAT` and `efficiencyFDH` require entering the used distance by the `score_model` argument. In case of `efficiencyEAT`, the `EAT` object is necessary. 
...
* `BCC_out`: Banker Charnes and Cooper output-oriented radial model.
* `BCC_in`: Banker Charnes and Cooper input-oriented radial model.
* `DDF`: Directional Distance Function.
* `RSL_out`: output-oriented Rusell Model.
* `RSL_in`: input-oriented Rusell Model.
* `WAM`: Weighted Additive Model.
...
```{r scores, collapse = FALSE, eval = FALSE}
...
```{r jitter_single, collapse = FALSE, fig.width = 10.5, fig.height = 8, fig.align = 'center', eval = FALSE}
...
```{r frontier_comparar, fig.width = 10.5, fig.height = 6, fig.align = 'center', eval = FALSE}
plot(frontier)
```
...
```{r density_single, collapse = FALSE, fig.width = 10.5, fig.height = 8, fig.align = 'center', eval = FALSE}
