...
eff_data <- function(data, tree, x, y, FDH = FDH) {
...
EAT_BCC_output <- function(data, tree, x, y, FDH = FALSE) {
  
 efficiency_data <- eff_data(data, tree, x, y, FDH = FDH)
...
efficiency_scores <- function(data, tree, x, y, scores_model, FDH = FALSE) {
  ...
  if (scores_model == "EAT_BCC_output"){
    ...
    if(FDH == TRUE)
      scores_FDH <- EAT_BCC_output(data, tree, x, y, FDH = TRUE)
