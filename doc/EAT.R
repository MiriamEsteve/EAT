...
predictionsRFEAT <- cbind(PISAindex[- 32, 3], predictions_RFEAT[, 6],
                          PISAindex[- 32, 4], predictions_RFEAT[, 7],
                          PISAindex[- 32, 5], predictions_RFEAT[, 8])

predictionsFDH <- cbind(PISAindex[- 32, 3], predictions_FDH[, 6],
                        PISAindex[- 32, 4], predictions_FDH[, 7],
                        PISAindex[- 32, 5], predictions_FDH[, 8])

kableExtra::kable(predictionsEAT) %>%
  kableExtra::kable_styling("striped", full_width = F) %>%
  kableExtra::column_spec(c(1, 3, 5), background = "#DBFFD6") %>%
  kableExtra::column_spec(c(2, 4, 6), background = "#FFFFD1")
