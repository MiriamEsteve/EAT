...
In this section, an **EAT** model, an **RFEAT** model, a **FDH** model and a **DEA** model refer to a modeling carried out using Efficiency Analysis Trees technique, Random Forest + Efficiency Analysis Trees technique, Free Disposal Hull method and Data Envelopment Analysis method respectively. The functions developed in the `EAT` library are always oriented to one of the four previous models (EAT, RFEAT, FDH or DEA) and can be divided into seven categories depending on its purpose:
...
The impurity of a given node `t` is measured as the prediction error at the node `t` over the total number of observations:
\begin{equation}
...
* `Primary & surrogate splits` where:
  ...
  * `variable --> {R: , s: }` represents the division of the space with its impurity and threshold.
  * `Surrogate splits` indicates the best possible split for each variable that has not been used to divide the node. This is expressed as `variable --> {R: , s: }` where `R` is the impurity at the node and `s` the threshold. Results are displayed in descending order by their `R` value. In the case of a single input, the surrogate splits do not appear.
...
`eff.levels()` returns the efficient levels of the outputs:
  
```{r effs.single.output, collapse = FALSE}
eff.levels(single_model)
```

`performance()` returns evaluation metrics for a Efficiency Analysis Trees model such as:

* `RMSE`: root mean square error at the model.
* `MAE`: mean absolute error at the model.
* `Bias`: bias at the model.
* `descriptive` where the following elements are showed for each node and output:
  * `Node`: node index.
  * `n(t)`: number of DMUs at the node.
...
* `varInfo` : list containing the impurity of the left node, the impurity of the right node and the threshold of the best split for each input.
* `R` : impurity at the node.
...
In this section, `PISAindex` is divided into a training subset with 70% of the DMUs and a test set with the other 30% of the DMUs. Next, the `bestEAT` function is applied to find the value of the hyperparameters `numStop` and `fold` that minimize the impurity calculated on the test sample from a Efficiency Analysis Tree trained with the training sample.
...
The `bestEAT` function requires a training set (`training`) on which to model an Efficiency Analysis Tree and a test set (`test`) on which to calculate the impurity. The number of trees built is given by the number of different combinations that can be given by the `numStop` and `fold` arguments. `bestEAT()` returns a data frame with the following columns:
  
  * `numStop` : numStop hyperparameter value.
  * `fold` : fold hyperparameter value.
  * `RMSE`: root mean square error calculated on the test sample with the tree built with the training sample, numStop and fold values.
...
The best tree is given by the hyperparameters `{numStop = 7, fold = 7}` in the reduced model with `RMSE = 53.55` and 10 leaf nodes.
...
`efficiencyJitter` returns a jitter plot from `ggplot2`. This graphic shows how DMUs are grouped into leaf nodes in a model built using the `EAT` function. Each leaf node groups DMUs with the same level of resources. The dot and the black line represent, respectively, the mean value and the standard deviation of the scores of its node. Additionally, efficient DMU labels always are displayed based on the model entered in the `score_model` argument. Finally, the user can specify an upper bound (`upb`) and a lower bound (`lwb`) in order to show, in addition, the labels which efficiency score is between them. Scores from a convexified Efficiency Analysis Tree (`CEAT`) model can also be used.
...
As in `bestEAT()`, the `bestRFEAT` function is applied to find the optimal hyperparameters that minimize the root mean square error calculated on the test sample. In this case, the available hyperparameters are `numStop`, `m` and `s_mtry`. Note that `s_mtry` is a character vector even if integer values are introduced.
