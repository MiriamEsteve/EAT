...
The `frontier` function displays the frontier estimated by EAT. Training DMUs can be included by `train.data = TRUE` and its color and shape can be modified with `train.color` and `pch` respectively. Finally, rownames can be included instead of points with `rownames = TRUE` and the label size can be changed with `rwn.size`.
...
```{r EAT_plot, fig.width = 10.5, fig.height = 9, fig.align = 'center'}
...
Additionally, `EAT` allows a feature selection. For this purpose, a histogram with the importance of each variable is displayed. Additionally, the user can vary the importance threshold in the graph with the argument `threshold` and the number of decimals units for importance with `r`.
...
```{r science_importance, fig.width = 10.5, fig.height = 6, fig.align = 'center', eval = F}
M_Breiman(data = training, tree = science_model, 
          x = input, y = output,
          threshold = 75)
```

```{r science_reduced_model, collapse = FALSE, eval = F}
# Input indexes
...
```{r science_reduced_model_plot, fig.width = 10.5, fig.height = 8, fig.align = 'center', eval = F}
...
```{r science_reduced_model_second, collapse = FALSE, eval = F}

science_reduced_model_second <- EAT(data = training, x = input, y = output,
                                    numStop = 8)
```

```{r science_reduced_model_second_plot, fig.width = 10.5, fig.height = 8, fig.align = 'center', eval = F}
...
```{r plot_multioutput, fig.width = 10.5, fig.height = 8, fig.align = 'center', eval = F}
...
```{r prediction, collapse = F, eval = F}
...
```{r scores,  fig.width = 10.5, fig.height = 8, fig.align = 'center', collapse = FALSE, eval = F}
