...
#'
barplot_importance <- function(m){

  Importance <- NULL

  ggplot(m, aes(x = row.names(m), y = Importance)) +
    geom_col() +
    xlab("Variable")
...
M_Breiman <- function(data, tree, x, y, r = 2) {
  ...
  for(var in 1:nX) {
    ...
    for(t in 1:length(tree)){
      ...
      errors <- resultado %>% filter(xi == var,
                                     id == tree[[t]][["id"]])
  ...
  return(list(m, barplot_importance(m)))
...
imp_var_Breiman <- function(data, tree, x, y, r){
  ...
  for(t in 1:length(tree)){
    ...
    for(xi in 1:nX) {
      ...
      array <- data[index, xi] %>% unique() %>% sort()

      for(i in 2:length(array)){

        if(tree[[t]][["s"]] == array[i]) next

        errLp_errRp_tLp_tRp <- estimEAT(data, tree, tree[[t]], xi,
                                        array[i], y)
        ...
        P <- append(P, list(list("id" = tree[[t]][["id"]],
                                 "xi" = xi,
                                 "s" = tree[[t]][["s"]],
                                 "array" = array[i],
                                 "errL" = errL_p,
      ...
      max_value <- lapply(P, function(x) {x$imp}) %>% unlist() %>% max()

      if(all(lapply(P, function(x) {x$imp}) == max_value) == TRUE){
...
#' @title Ranking
#'
#' @description This function evaluates the importance of each predictor variable.
#'
#' @param data Data to be used.
#' @param tree Tree-object of EAT function.
#' @param x Column input indexes in data.
...
#'
#' @return Dataframe with one column and the importance of each variable in rows.
Ranking <- function(data, tree, x, y, r = 2) {
  ...
  for(xi in 1:nX){
    ...
    for(t in 1:length(tree)){
      ...
      M[[xi]] <- M[[xi]] + (tree[[t]][["R"]] - tree[[t]][["varInfo"]][[xi]][[1]] - tree[[t]][["varInfo"]][[xi]][[2]])
