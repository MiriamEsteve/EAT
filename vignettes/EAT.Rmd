...
Random Forest (`RFEAT`) has also been developed with the aim of providing greater stability to the results obtained by `EAT`. Now, a new argument, `m`, is introduced that indicates the number of `EAT` individual trees that are used to build the forest. `s_mtry` indicates the number of inputs that can be selected in each split. Being, $n_{x}$ the number of inputs, $n_{y}$ the number of outputs and $N$ the sample size, the available options in `s_mtry` are:
