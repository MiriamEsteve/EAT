...
As any other Machine Learning algorithm, `EAT` performs predictions by `predict` function. `predict` returns a `data.frame` with the expected output(s) for a set of observations. `::` should be used to avoid conflicts between packages. The arguments to the `predict` function are `t` (an `EAT` object) and `newdata` (dataframe with input variables). 
...
Esta función hay que mejorarla bastante

```{r barplot_scores, collapse = FALSE, fig.width = 10.5, fig.height = 8, fig.align = 'center'}
...
```{r efficiency_density, collapse = FALSE, fig.width = 10.5, fig.height = 8, fig.align = 'center'}
