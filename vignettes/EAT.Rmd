...
functions <- data.frame("Purpose" = c(rep("Modeling", 2),
                        ...
                        "Usage" = c("Apply Efficiency Analysis Trees technique to a data frame. Return an EAT object.",
                                    "Apply Random Forest + Efficiency Analysis technique to a data frame. Return a RFEAT object.",
...
`plotRFEAT()` returns the root mean square error (RMSE) for the training data set for the number of trees given on the x-axis.

```{r plot.RFEAT, collapse = FALSE, fig.width = 7.2, fig.height = 6, fig.align = 'center'}
plotRFEAT(forest)
```
